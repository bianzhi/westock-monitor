...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    if git pull --ff-only > /dev/null 2>&1; then
        ...
        echo "   ⚠️  git pull 失败（可能有本地改动/网络问题），继续用当前代码"
    fi
else
    echo "   ℹ️  非 git 仓库，跳过 pull"
fi
...
# ----------------------------------------------------------
# 4.5 后台补采流通市值（方案 C 腾讯落库涨跌幅/换手率）
# ----------------------------------------------------------
# 修复 circ_mv loop 未到触发时点（09:15/15:05）导致两列空：
# 启动后立即后台触发一次，避免等到下个时点。
echo ""
echo "📊 后台补采流通市值（方案 C 腾讯，落库涨跌幅/换手率）..."
nohup "$VENV_PYTHON" -c "from collector import collect_circ_mv_snapshot; collect_circ_mv_snapshot()" \
    > "$LOGDIR/circ_mv_backfill.log" 2>&1 &
echo "   已后台触发（日志: $LOGDIR/circ_mv_backfill.log，约 30-60s 完成）"
