...
强度判定的输入是**净额率**（主力净流入 ÷ 成交额），分两种口径：

- **单日净额率** = 当日主力净流入 ÷ 当日成交额 × 100%（宽表「今日净额率」列）
- **近 n 日聚合净额率** = 近 n 日净流入之和 ÷ 近 n 日成交额之和 × 100%（**强度判定实际用这个**）

> ⚠️ 与「资金强度」区分：资金强度 = 当日主力净流入 ÷ 流通市值 × 100%，
> 分母是**流通市值**而非成交额，用于跨市值归一化比较，两者不是一回事。
...
阈值关系恒为 **hi > mid > 0 > lo > vlo**（`lo`/`vlo` 是负数）。

### 5 档判定

设近 n 日聚合净额率为 `nr`，与所属规模档位的 4 个阈值比较：
...
> 因为 `lo` 是负数，「普通」档横跨 `[lo, mid)`，净额率略负仍算普通；
> 只有 `nr < vlo` 才是「弱」。数据不足时默认「普通」。
...
```
nr ≥ hi        → +2.0
mid ≤ nr < hi  → [1.0, 2.0)   线性插值
0   ≤ nr < mid → [0, 1.0)     线性插值
lo  ≤ nr < 0   → [-1.0, 0)    线性插值
vlo ≤ nr < lo  → [-2.0, -1.0) 线性插值
nr < vlo       → -2.0
```
